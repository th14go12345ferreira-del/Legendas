...
Java_br_com_thiago_legendaoffline_WhisperBridge_transcribe(
        JNIEnv* env, jobject, jstring modelPath, jfloatArray audio, jstring language, jint threads) {
    ...
    if (!ctx) {
        ...
        return env->NewStringUTF("ERROR|Falha ao carregar o modelo Whisper.");
    }
